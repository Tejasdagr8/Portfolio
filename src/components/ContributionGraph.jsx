...
function DayCell({ day }) {
  ...
  return (
    <span
      ...
      className={`w-[10px] h-[10px] sm:w-[11px] sm:h-[11px] rounded-[2px] shrink-0 transition-transform hover:scale-125 ${
        highlighted ? "ring-1 ring-mint/50 ring-offset-0" : ""
      }`}
      style={{
...
export default function ContributionGraph({ contributions, total, phaseTotal, loading }) {
  ...
  const phasePct = total > 0 ? Math.round((phaseTotal / total) * 100) : 0;
    ...
    <div className="rounded-xl border border-white/[0.08] bg-ink-0/40 px-3.5 py-4 space-y-3">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="font-mono text-[11px] text-paper">
            <span className="text-mint tabular-nums">{total.toLocaleString()}</span> contributions
            <span className="text-fog/50"> · last year</span>
          </p>
          <p className="font-mono text-[10px] text-fog/70 mt-1">
            <span className="text-ember tabular-nums">{phaseTotal.toLocaleString()}</span>
            {" "}since late Mar ·{" "}
            <span className="text-mint">{phasePct}%</span> of activity
          </p>
        </div>
        <div className="flex items-center gap-2 font-mono text-[9px] text-fog/60 uppercase tracking-wider">
          <span className="inline-flex items-center gap-1.5">
            <span className="w-2.5 h-2.5 rounded-[2px] bg-white/[0.06] opacity-30" /> quiet
          </span>
          <span className="inline-flex items-center gap-1.5">
            <span className="w-2.5 h-2.5 rounded-[2px] ring-1 ring-mint/50" style={{ backgroundColor: LEVEL_COLORS[4] }} />
            shipping phase
          </span>
        </div>
      </div>
      ...
      <div className="relative overflow-x-auto pb-1 -mx-1 px-1">
        <div className="min-w-[620px]">
          <div className="flex gap-[3px] mb-1 pl-[18px] h-3">
            {weeks.map((_, wi) => {
          ...
          <div className="relative flex gap-[3px]">
            {weeks.map((week, wi) => {
              const inPhase = week.some((d) => d && isHighlighted(d.date));
              return (
                <div
                  key={`w-${wi}`}
                  className={`flex flex-col gap-[3px] shrink-0 rounded-[3px] ${
                    inPhase ? "bg-mint/[0.06] ring-1 ring-mint/15 -mx-[1px] px-[1px]" : ""
                  }`}
                >
                  {week.map((day, di) => (
      ...
      <p className="font-mono text-[9px] text-fog/50">
        Highlighted band · late Mar 2026 → now · combined public activity across both accounts
      </p>
    </div>
  );
}
